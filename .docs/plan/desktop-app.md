# Peektrace Desktop App + Distributable CLI — Implementation Plan

Wrap peektrace (Effect Bun CLI + React/Vite inspector) into: (1) a distributable desktop app, (2) an npm-installable compiled CLI. Mirrors the `executor` open-source project.
...
| Question | Answer |
|---|---|
| appId | `com.mark-life.peektrace` · productName `Peektrace` |
| Repo (public) | `Mark-Life/peektrace` — updater feed owner=`Mark-Life` repo=`peektrace` |
...
```
 ...
 (thin wrapper)  │  - single-instance lock, window-state    │
                 ...
                 └───────────────┬─────────────────────────-┘
                                 │ spawn + PEEKTRACE_READY:<port> handshake
                                 ▼
                 ┌─────────────────────────────────────────┐
 sidecar =       │ compiled `peektrace` binary               │
 same binary     │  (bun build --compile of apps/cli)       │
 npm also ships  │  `peektrace serve --port N --client desktop`│
...
Single artifact, two roles: the compiled `peektrace` binary is BOTH the npm CLI and the desktop sidecar. Everything hinges on Phase A producing that binary with the inspector baked in.
...
- `apps/cli/package.json` `bin.peektrace` → `./src/index.ts` (raw TS, no compile)
...
**STATUS: DONE** — compiled `peektrace` binary serves embedded inspector with zero external files; all validation checks pass (builds, /health ok, root HTML, PEEKTRACE_READY sentinel, runs from /tmp, RPC works). Binary builds at `apps/cli/dist/<target>/peektrace`. Files: `apps/cli/src/build.ts`, `apps/cli/src/embedded-ui.gen.ts`, `apps/cli/src/commands/serve.ts`, `apps/cli/package.json`.

**Goal:** `bun build --compile apps/cli/src/index.ts` → single `peektrace` binary that serves the inspector with zero external files. This is the load-bearing phase — validate in isolation before any Electron work.
...
2. **`serve.ts` static handler — resolve order:** `PEEKTRACE_CLIENT_DIR` env (dev override) → embedded map (packaged) → relative `DIST_DIR` (source dev fallback). Fixes `:39-41`. — done
3. **`apps/cli/src/build.ts` compile command:** `bun build --compile --target=<BUN_TARGET|host> --outfile dist/<target>/peektrace apps/cli/src/index.ts`. No native `.node`/WASM in peektrace → simpler than executor (no sibling-file staging). — done
4. **Ready sentinel:** after server listen, if `process.env.PEEKTRACE_CLIENT === "desktop"` → `console.log("PEEKTRACE_READY:" + port)`. Keep existing human log. — done
...
**Acceptance:**
- `apps/cli/dist/<host>/peektrace serve --port 7777` boots, `curl 127.0.0.1:7777/health` → `ok`, `curl 127.0.0.1:7777/` → inspector HTML, inspector loads + RPC works — **run from an unrelated cwd** (proves no path deps), copy binary to `/tmp` and run there too.
- `PEEKTRACE_CLIENT=desktop peektrace serve --port 7777` prints `PEEKTRACE_READY:7777`.
...
| `src/main/sidecar.ts` | `src/main/sidecar.ts` | spawn, `PEEKTRACE_READY:<port>` parse, `/health` probe, SIGTERM→(5s)→SIGKILL on `before-quit`, expected-exit set | supervised-daemon attach, manifest |
...
2. Dev sidecar cmd: `bun run apps/cli/src/index.ts serve --port <p> --no-open` with env `PEEKTRACE_CLIENT=desktop`. (Ensure `serve` has a `--no-open`/`--client` flag; add if missing.) — done (`--no-open` flag present in `serve.ts`)
...
**Acceptance:** `bun run --filter=@workspace/desktop dev` opens a window rendering the inspector; closing the app kills the sidecar (no orphan `peektrace`/`bun` process — check `ps`). — TODO: human-only runtime verification (window renders inspector; no orphan procs on quit) not yet run.

---

# PHASE C — Packaging (electron-builder)

**STATUS: PARTIAL** — C.1 implemented; the `electron-builder.config.ts` loads cleanly and the host sidecar is staged (`apps/desktop/resources/peektrace/peektrace`, chmod 0o755). `build-sidecar.ts` runs the CLI's own `src/build.ts` (host target, or `BUN_TARGET` cross target) and copies the compiled binary into `resources/peektrace/`. Config sets `appId: com.mark-life.peektrace`, `productName: Peektrace`, `artifactName: peektrace-desktop-${os}-${arch}.${ext}`, `directories.output: dist-app`, `extraResources` for `resources/peektrace/`, and `mac: { target: ["dmg","zip"], hardenedRuntime: true, entitlements: build/entitlements.mac.plist, notarize: false }` (unsigned). Main resolves the packaged sidecar at `process.resourcesPath/peektrace/peektrace`. Files: `apps/desktop/electron-builder.config.ts`, `apps/desktop/scripts/build-sidecar.ts`, `apps/desktop/build/entitlements.mac.plist`, `apps/desktop/build/icon.png`.
...
**C.1 — mac first**
1. `build-sidecar.ts` (from executor): run `apps/cli` `build.ts` for host target → copy `apps/cli/dist/<target>/peektrace` → `apps/desktop/resources/peektrace/` + `chmod 0o755`. — done
2. `electron-builder.config.ts`:
   - `appId: "com.mark-life.peektrace"`, `productName: "Peektrace"`
   - `artifactName: "peektrace-desktop-${os}-${arch}.${ext}"`
   - `extraResources: [{ from: "resources/peektrace/", to: "peektrace/" }]` (outside asar)
...
4. Main resolves packaged sidecar at `process.resourcesPath/peektrace/peektrace` (dev: `bun run …`). — done
5. `build/icon.png` — 1024×1024 RGBA peektrace icon (placeholder ok initially). — done (placeholder committed)
...
**STATUS: DONE** — all wiring in place and typechecks. `apps/desktop/package.json` has `dev`, `build` (`bun run scripts/build-sidecar.ts && electron-vite build`), `package`/`package:mac`/`package:win`/`package:linux`, `preview`, `predev`/`prebuild` (inspector build), `typecheck`. Root `turbo.json` has `@workspace/desktop#build` depending on `inspector#build` + `peektrace#build:binary` (plus a `build:binary` task). Root `package.json` keeps `serve` unchanged and adds `desktop:dev` + `desktop:package`. Files: `apps/desktop/package.json`, `turbo.json`, `package.json`.
...
- Root `turbo.json`: `@workspace/desktop#build` depends on `inspector#build` + `peektrace#build:binary`. Add root scripts `desktop:dev`, `desktop:package`. — done
...
**STATUS: DONE** (local staging + generation; actual `npm publish` is external, deferred to CI/Phase H). Naming resolved **unscoped** — wrapper `peektrace` + per-platform `peektrace-<platform>-<arch>` variants (no npm org needed). Generator `scripts/build-npm.ts` cross-compiles all four targets via `BUN_TARGET` (re-running Phase A's `src/build.ts`) and stages clean publishable dirs under gitignored `dist-npm/`: main `peektrace/` (Node CommonJS `bin` shim `peektrace.js` + `postinstall.js` + `optionalDependencies` on all variants, `os`/`cpu` filtered) and one `peektrace-<platform>-<arch>/` per binary (`os`/`cpu` set so npm fetches only the host match). Shim resolves the per-platform optional dep via `require.resolve`, execs `bin/peektrace[.exe]`, forwards argv/stdio/signals, propagates exit code; honors `PEEKTRACE_BIN_PATH` escape hatch. `serve.ts` gained a `--host` option (default `127.0.0.1` loopback; `--host 0.0.0.0` binds all interfaces with a no-auth WARNING; dial-host normalization for wildcard binds). Validation: pass=true — generatorRuns, stagedPackagesValid, shimExecsHostBinary, npmPackDryRun, devStillInstalls all pass; per-target builds: darwin-arm64 65.4MB, darwin-x64 70.6MB, linux-x64 104.2MB (cross-compiled from macOS), win32-x64 120.0MB (cross-compiled, `bin/peektrace.exe`).

Created: `apps/cli/scripts/build-npm.ts`, `apps/cli/scripts/npm/peektrace.js`, `apps/cli/scripts/npm/postinstall.js`, `apps/cli/scripts/npm/README.md`, `apps/cli/README.md`. Edited: `apps/cli/package.json` (add `build:npm` script), `apps/cli/src/commands/serve.ts` (add `--host`), `apps/cli/tsconfig.json`, `.gitignore` (ignore `dist-npm/`).

**Remaining (external):** actual `npm publish` (needs `NPM_TOKEN`, publish each `peektrace-*` first then `peektrace`) happens in CI / Phase H — not run locally. No npm org required (unscoped).

**Goal:** `npm i -g peektrace` installs the compiled binary on macOS/Linux/Windows servers.

**Pattern (executor's):** thin main package + per-platform optional-dependency binary packages.
- Main package `peektrace` (from `apps/cli`): `bin` → a tiny JS shim that resolves + `execFileSync` the right `@peektrace/cli-<os>-<arch>` binary from `optionalDependencies`. `postinstall` verifies presence.
- Per-platform packages `@peektrace/cli-darwin-arm64`, `-darwin-x64`, `-linux-x64`, `-win32-x64`, each containing one compiled binary, with `os`/`cpu` fields so npm only fetches the matching one.
- Publish all from CI (Phase H) on `cli-v*` tag.

**Steps:**
1. Verify executor's exact layout: `apps/cli/package.json` (`bin` shim, `optionalDependencies`), its per-platform package generator script, and `scripts/build.ts` cross-compile targets. Copy-adapt. — done (adapted as `scripts/build-npm.ts` + `scripts/npm/{peektrace.js,postinstall.js}`; wrapper staged to `dist-npm/` rather than mutating the private `apps/cli/package.json`)
...
3. Linux server just needs the binary + no display → `peektrace serve` runs headless; user opens `http://<vps-ip>:<port>` (document binding `--host 0.0.0.0` + firewall caveat; loopback-only by default). — done (`--host` option added to `serve.ts`, default loopback, `0.0.0.0` prints no-auth WARNING)

**Note:** this reuses Phase A's binary exactly — no new compile logic, just packaging + publish.

**Open:** RESOLVED — unscoped `peektrace-<platform>-<arch>` (+ wrapper `peektrace`); no npm org needed. Scoped `@peektrace/*` was the rejected alternative.
...
2. `setupAutoUpdater()` (from executor `index.ts`): **packaged-only**, `electron-updater`, `provider: "github", owner: "Mark-Life", repo: "peektrace"`, ~4h poll, "Restart to update" dialog → `quitAndInstall`. — done
3. `electron-builder.config.ts`: `publish: { provider: "github", owner: "Mark-Life", repo: "peektrace" }` → emits `latest*.yml` manifests. — done
...
Peektrace has **no Changesets** (executor does) → simpler. Independent tags per artifact.
...
- Each leg: `bun install --frozen-lockfile` → `bun run --filter=inspector build` → `bun apps/desktop/scripts/build-sidecar.ts` (with `BUN_TARGET` for the leg) → **smoke-test binary** (`peektrace serve` boots + `/health` 200) → `electron-vite build` → `electron-builder --<os> --<arch> --publish never`. — done
...
- Cross-compile per target → publish each `@peektrace/cli-<os>-<arch>` + main `peektrace` to npm (needs `NPM_TOKEN` secret — external/manual). Version from tag; tracks `apps/cli/package.json`. — done
...
| **E** | `npm i -g peektrace` (VPS) | 1–2 d | reuses A binary |
...
## Template map (executor absolute paths → peektrace)
...
- npm scope: **RESOLVED = unscoped** `peektrace-<os>-<arch>` (+ wrapper `peektrace`); no npm org needed. `@peektrace/*` scoped family rejected.
