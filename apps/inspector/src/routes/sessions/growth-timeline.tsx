/** Context-growth timeline (Phase 8.2) — React/SVG port of `lib/svg.ts`.
 *
 * A stacked-area chart of real per-turn context (floor → top by `STACK_ORDER`),
 * over a 0→window y-axis, with: the dumb-zone danger band + boundary line, the
 * 200K-model ghost ceiling, compaction cliffs, the peak marker, and the first
 * dumb-zone crossing marker. Geometry mirrors the report's viewBox so the shape
...
export const GrowthTimeline = ({ a }: { readonly a: AnalyzedSession }) => {
