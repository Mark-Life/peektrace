# Peephole Inspector (`apps/inspector`)

Vite + React 19 + Tailwind v4 UI for Peephole — a loopback-only forensic
...
src/
  ...
  lib/
    client.ts                   the single AtomRpc client (PeepholeClient)
...
export class PeepholeClient extends AtomRpc.Tag<PeepholeClient>()(
  "PeepholeClient",
  { group: PeepholeRpcs, protocol } // protocol = HTTP/NDJSON → `${BASE_URL}/rpc`
) {}
```

Each procedure becomes a `Result`-typed atom:

- `PeepholeClient.query(tag, payload)` → `Atom<Result<Success, Error>>`
- `PeepholeClient.mutation(tag)` → writable `AtomResultFn` (for the Memory CRUD
  phase)
...
`VITE_PEEPHOLE_BASE_URL` to point the UI at a remote `peephole serve`.
...
with `PEEPHOLE_RPC_TARGET`. HMR + the live RPC backend run side by side.

### Prod — same-origin, served by `peephole serve`
...
`peephole serve` serves the static `dist/` at `/` (SPA fallback to
...
`PEEPHOLE_CLAUDE_PROJECTS` to redirect it at a temp dir (used by automated tests
